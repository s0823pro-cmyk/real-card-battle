...
export const useRunProgress = () => {
  ...
  const openRankingFromHome = () => {
    dispatch({ type: 'set_screen', screen: 'ranking' });
  };

  const backToHomeFromRanking = () => {
    dispatch({ type: 'set_screen', screen: 'home' });
  };
  ...
  const startDevNavigation = (
    destination: Exclude<DevDestination, 'boss_reward' | 'story' | 'debug_zukan_cards' | 'debug_zukan_stories'>,
  ) => {
    const jobId: JobId =
